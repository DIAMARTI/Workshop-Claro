...
## **Crear el entorno de trabajo entre el servidor y los clients**
...
```
[root@client81 ~]# subscription-manager repos --enable=ansible-2.9-for-rhel-8-x86_64-rpms
[root@client81 ~]# yum install platform-python -y
---

5. Crear el usuario ansible en el nodo de control y asignarle el password redhat, verifique que el UID es el mismo del servidor, en caso de no coincidir pida apoyo al instructor.


<br>[root@client81 ~]# useradd ansible
<br>[root@client81 ~]# id ansible
<br>uid=1000(ansible) gid=1000(ansible) groups=1000(ansible)
<br>[root@client81 ~]# echo "redhat" | passwd stdin ansible


6. Configurarle permisos sudo al usuario ansible en el nodo de control
```
[root@client81 ~]# echo "ansible ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/ansible

[root@client81 ~]# cat /etc/sudoers.d/ansible
ansible ALL=(ALL)       NOPASSWD: ALL
```

Repita los pasos 4 5 y 6 en cada cliente de su infraestructura, clientX1, clientX2, clientX3 y clientX4
